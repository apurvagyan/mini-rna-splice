...
def process_record(record):
    sequence = record[0].split(': ')[1].strip()
    secondary_structure = record[1].split(': ')[1].strip()
    min_free_energy = float(record[2].split(': ')[1].strip())
    
    adj_mat = dot2adj(secondary_structure)
    adj_list = adj_matrix_to_adj_list(adj_mat)
    
    return sequence, min_free_energy, adj_list
...
def write_data(results, lock, fnames):
    for data in results:
        sequence, min_free_energy, adj_list = data
...
def main():
    ...
    fnames = [seq_fname, adj_list_fname, energy_fname]
